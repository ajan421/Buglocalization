...
def step_parse_code(source_dirs, output_file = "code_structure.json") -> bool:
    """
...
def step_build_knowledge_graph(parsed_file = "code_structure.json") -> Neo4jKnowledgeGraph:
    """
...
def step_localize_bugs(knowledge_graph: Neo4jKnowledgeGraph, bug_reports_dir = "bug_reports", num_top_locations: int = 5, output_dir = None):
    """
    ...
    # Save results
    if output_dir:
        output_file = Path(output_dir) / "bug_localization_results.json"
    else:
        output_file = Path(bug_reports_dir).parent / "outputs/json/bug_localization_results.json"
...
def main():
    ...
    source_dirs = [
        project_root / "aspectj/weaver/src",
        project_root / "aspectj/runtime/src",
        project_root / "aspectj/ajde/src",
        project_root / "aspectj/ajde.core/src",
        project_root / "aspectj/org.aspectj.ajdt.core/src",
        project_root / "aspectj/bridge/src",
        project_root / "aspectj/bcel-builder/src",
        project_root / "aspectj/asm/src"
    ]
    
    # Output files
    code_structure_file = project_root / "outputs/json/code_structure.json"
    ...
    (project_root / "outputs/json").mkdir(parents=True, exist_ok=True)
    ...
    step_localize_bugs(knowledge_graph, bug_reports_dir, num_top_locations, project_root / "outputs/json")
    ...
    print(f"  Bug reports: {bug_reports_dir}")
    print(f"  Results: {project_root / 'outputs/json/bug_localization_results.json'}")
